...
def _create_pull_request(branch_name: str, task_id: str, task_name: str, file_count: int) -> str:
    """Create a PR from branch into base branch and return the PR URL."""
    url = f"https://api.github.com/repos/{GITHUB_REPO}/pulls"
    payload = {
        "title": f"feat: scaffold code for ClickUp task {task_id}",
        "body": (
            f"## Auto-generated by ClickUp Automation\n\n"
            f"**ClickUp Task ID:** `{task_id}`\n"
            f"**Task:** {task_name}\n\n"
            f"### Generated Files\n"
            f"{file_count} file(s) scaffolded under `scaffolded/{task_id}/`\n\n"
            f"> This PR was automatically created when the task status changed in ClickUp."
        ),
        "head": branch_name,
...
def process_clickup_task(task_id: str):
    ...
    try:
        ...
        # 6. Auto-create PR against base branch
        pr_url = _create_pull_request(branch_name, task_id, task_name, len(files))
        print(f"🔗 {pr_url}")
        return True
